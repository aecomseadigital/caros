...
pub async fn handle_session_export(
    ...
    if nostr {
        return Err(anyhow::anyhow!("caros was not built with nostr support"));
    }
...
pub async fn handle_session_import(input: String, nostr: bool) -> Result<()> {
    let json = if nostr || input.starts_with("caros://sessions/nostr") {
        #[cfg(feature = "nostr")]
        {
    ...
    let label = match format {
        goose::session::import_formats::ImportFormat::Goose => "Caros",
