...
get_latest_release() {
    if command -v gh &> /dev/null; then
        gh release list --repo yixuanzhong/caros --limit 1 --json tagName --jq '.[0].tagName' 2>/dev/null
    else
...
get_previous_release() {
    if command -v gh &> /dev/null; then
        gh release list --repo yixuanzhong/caros --limit 2 --json tagName --jq '.[].tagName' 2>/dev/null | sed -n '2p'
    else
