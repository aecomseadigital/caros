    ...
    if (isUsingGitHubFallback) {
      ...
      try {
        ...
        const dialogResult = (await dialog.showMessageBox({
          ...
          detail: `The update has been downloaded and extracted. To complete the installation:\n\n1. Click "Open Folder" to view the new Caros.app\n2. Quit Caros (this app will close)\n3. Drag the new Caros.app to your Applications folder\n4. Replace the existing app when prompted\n\nThe update will be available the next time you launch Caros.`,
...
export function setupAutoUpdater(tray?: Tray) {
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    const notification = new Notification({
      title: 'Update Ready',
      body: `Version ${info.version} will be installed when you quit Caros. Click to install now.`,
    });
    notification.show();
